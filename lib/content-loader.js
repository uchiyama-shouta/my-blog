...
const lifeDIR = path.join(process.cwd(), "content/posts/life");
const businessDIR = path.join(process.cwd(), "content/posts/business");
const EXTENSION = ".md";
...
const listLifeContentFiles = ({ fs }) => {
	const filenames = fs.readdirSync(lifeDIR);
	return filenames.filter((filename) => path.parse(filename).ext === EXTENSION);
};
const listBusinessContentFiles = ({ fs }) => {
	const filenames = fs.readdirSync(businessDIR);
	return filenames.filter((filename) => path.parse(filename).ext === EXTENSION);
};

const listAllContentFiles = async ({ fs }) => {
	const contentFilenames = await listContentFiles({fs});
	const programmingContentFilenames = await listContentFiles({fs});
	const lifeContentFilenames = await listContentFiles({fs});
	const businessContentFilenames = await listContentFiles({fs});
	const allFilenames = {
		...contentFilenames,
		...programmingContentFilenames,
		...lifeContentFilenames,
		...businessContentFilenames,
	}
	return allFilenames;
};
...
const readLifeContentFile = async ({ fs, slug, filename }) => {
	if (slug === undefined) {
		slug = path.parse(filename).name;
	}
	const raw = fs.readFileSync(
		path.join(lifeDIR, `${slug}${EXTENSION}`),
		"utf8"
	);
	const matterResult = matter(raw);
	const { title, published: rawPublished, category } = matterResult.data;
...
const readBusinessContentFile = async ({ fs, slug, filename }) => {
	if (slug === undefined) {
		slug = path.parse(filename).name;
	}
	const raw = fs.readFileSync(
		path.join(businessDIR, `${slug}${EXTENSION}`),
		"utf8"
	);
	const matterResult = matter(raw);
	const { title, published: rawPublished, category } = matterResult.data;
...
const readLifeContentFiles = async ({ fs }) => {
	const promisses = listLifeContentFiles({ fs }).map((filename) =>
		readLifeContentFile({ fs, filename })
	);
	const contents = await Promise.all(promisses);
	return contents.sort(sortWithProp("published", true));
};
const readBusinessContentFiles = async ({ fs }) => {
	const promisses = listBusinessContentFiles({ fs }).map((filename) =>
		readBusinessContentFile({ fs, filename })
	);
	const contents = await Promise.all(promisses);
	return contents.sort(sortWithProp("published", true));
};
...
const readAllContent = async ({ fs }) => {
	...
	// life
	const life = await listLifeContentFiles({ fs }).map((filename) => {
		readLifeContentFile({ fs, filename });
	});
	const lifeContents = await Promise.all(life);

	// business
	const business = await listBusinessContentFiles({ fs }).map((filename) =>
		readBusinessContentFile({ fs, filename })
	);
	const businessContents = await Promise.all(business);
...
export {
	...
	listLifeContentFiles,
	readLifeContentFiles,
	readLifeContentFile,
	listBusinessContentFiles,
	readBusinessContentFiles,
	readBusinessContentFile,
	readAllContent,
	listAllContentFiles,
};
